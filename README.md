...
- **Start of the Day** — shows one image per weekday (`data/sotd/monday.png` … `friday.png`), autonomously picking today's. Unlike the other widgets, its content is images rather than text, so instead of typing into a form, the admin page's widget editor has 5 day slots plus a "whole week" batch uploader; picking a file commits it straight to GitHub immediately (no separate Save step), and each slot shows when it was last updated (from GitHub's commit history for that file) so it's obvious at a glance what's stale. Meant for slides exported from PowerPoint as PNG (File → Export → Change File Type → PNG → Save Every Slide) — exporting via PowerPoint itself, rather than any automated conversion, keeps images/maths symbols pixel-accurate. Default size is 2 columns × 2 rows; the image is displayed with `object-fit: contain`, so a 16:9 slide is never cropped or stretched regardless of the box it's given.
...
A widget whose configuration is files rather than form fields (like Start of the Day's images) can provide `initAdminForm(container, ctx)` instead of relying only on `readAdminForm()`. It's called once per widget instance after the admin page renders `renderAdminForm()`'s markup, and `ctx` gives it `commitFile(path, base64Content, message)` and `getLastCommitDate(path)` (both backed by the GitHub API using the admin's saved token) plus `imageUrl(path)` for previewing an already-published file. Such a widget's `readAdminForm()` typically just returns `{}`, since nothing it manages lives in `config.json`.
